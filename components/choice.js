export default function Choice ({choices,choice,removeChoice,moveUp,moveDown,hidden,voting}) {
    ...
    return (
        <div>
            ...
            <button className={`text-purple-600 shadow rounded-sm w-4 ml-2 font-bold text-center y-auto ${voting === true ? "hidden" : ""} ${hidden === true ? "hidden" : ""}`} onClick={() => removeChoice(choice)}> x </button>
            <br></br>
        </div>
    )
}